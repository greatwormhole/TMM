...
def alpha_alpha(rb, r):                              # Угол профиля на окружносте вершин
    return np.arccos(rb / r)
...
def analyze_single_condition(q1, q2, **kwargs):      # Определение параметров при фиксированном x1, x2
    ...
    rb1 = rad_osn(kwargs["module"], kwargs["z1"], kwargs["alpha"])
    rb2 = rad_osn(kwargs["module"], kwargs["z2"], kwargs["alpha"])
    alpha_alpha1 = alpha_alpha(rb1, r1) * 180 / np.pi
    alpha_alpha2 = alpha_alpha(rb2, r2) * 180 / np.pi
    ...
    res = str(f"rof: {rof_t:> 5.3f} мм\n"
          f"Минимальное количество зубьев: {int(zmin):> 5}\n"
          ...
          f"Коэффицент воспринимаемого смещения: {y_t:> 5.3f} мм\n"
          f"Коэффицент уравнительного смещения: {dy_t:> 5.3f} мм\n"
          f"Радиус первого колеса по делительной окружности: {r1:> 5.3f} мм\n"
          f"Радиус второго колеса по делительной окружности: {r1:> 5.3f} мм\n"
          f"Радиус первого колеса по основной окружности: {rb1:> 5.3f} мм\n"
          f"Радиус второго колеса по основной окружности: {rb1:> 5.3f} мм\n"
          ...
          f"Коэффицент удельного давления: {thetta_t:> 5.3f} DEG\n")

    
    print(res, f"\n\nДанные сохранены в TMM/data/results-{q1}-{q2}.txt\n\n")
    with open(f"data/results-{q1}-{q2}.txt", mode="w", encoding="UTF-8") as f:
        f.write(res)
